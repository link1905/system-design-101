...
While this helps maintain data integrity, it often makes SQL queries less efficient,
...
- **Eviction**: If memory is full, the least recently accessed pages are evicted to make room for new on.

## Indexing

We’ve previously discussed how indexes are organized.
In runtime, how an index is utilized depends heavily on the query context.
There are typically three common query pattern:
...
In large datasets, this back-and-forth can become inefficient.  
The high number of I/Os — jumping between index pages and heap pages — can noticeably degrade performance.
...
i: Index {
    ...
    grid-columns: 1
    i2: (Id = 1, Name = John)
    i2: (Id = 3, Name = Caitlyn)  {
        style.fill: ${colors.i2}
    }
    i3: (Id = 5, Name = Caitlyn)
}
query -> i.i2
```

However, note that adding extra columns makes the index larger.
It also complicates updates — any change to an included column requires updating the index entry as well,
making optimizations like [HOT updates](../physical-layer/#hot-update) impossible not feasible).
...
For example,
if most students have a `GPA > 6`, the system might opt for a table scan to retrieve those records.
...
Since indexes are typically smaller than table data, loading index pages first allows the system to gather a lot of positional information with fewer I/Os.
...
- The index might mark `Page 1` and `Page 2`.
- The system then reads these pages sequentially to extract the qualifying recordss.
...
**Bitmap scans** can also combine multiple index conditions using **bitwise operations** on their respective bitmaps.
For instance:
...
grid-columns: 1
query: SELECT Name WHERE Id < 7 AND Grade > 7 {
  shape: text
  near: top-center
  style: {
    font-size: 30
...
data: "" {
    ...
    i2: Grade Index {
        ...
        i5: (Grade = 6) -> (Page = 2, Offset = 1) 
        i4: (Grade = 8) -> (Page = 3, Offset = 1) {
            style.fill: ${colors.i2}
        }   
    }
...
Instead, a component called the **Query Planner** estimates and selects the most efficient query model based on cost:
...
So the planner estimates **2 rows**, illustrated by the following diagram:
...
It’s important to understand that both **MVC** and histogram metrics are derived from **randomly sampled** records within the table. As a result, the collected statistics may not perfectly represent the actual distribution of data across the entire table. While this estimation process isn’t flawless, it provides the database with enough insight to make reasonably informed decisions when selecting the most efficient query execution strategy.
...
Here, the main `User` table acts as a **proxy** to the underlying partitions.

- Improved performance by querying smaller, more focused tables.
- Reduced chances of [race conditions](../concurrency-control/#race-condition).
...
To optimize, we can precompute and store the GPA within the `Student` table itself:
...
Denormalization enables faster reads and is a fundamental principle in many [NoSQL databases](NoSQL-Databases.md).
...
- Broader transactions involving more updates, increasing chances of [concurrent conflicts and locking](../concurrency-contrl/).
