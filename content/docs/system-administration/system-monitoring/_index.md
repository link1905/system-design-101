...
- **Push Model**: This model entails installing an agent on the services being tracked.
...
The pull model may lead to **data loss** (for instance, if a service crashes before its data can be pulled),
...
s.s1.i -> m: Pull metrics {
  style.animated: true
}
s.s2.i -> m: Pull metrics {
  style.animated: true
}
```
